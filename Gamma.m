function G = Gamma(dq, q, os, ps)

% Wej�cie:
%   dq - pochodne wspó³rzêdnych absolutnych wzglêdem czasu,
%   q  - wspó³rzêdne absolutne uk³adu wielocz³onowego,
% Wyj�cie:
%   G - wektor gamma

Om = [0 -1; 1 0]; % Stala macierz

G = zeros(2*(size(os,1)+size(ps,1)), 1);

ind=1;

for k=1:size(os,1) %petla po wszystkich parach obrotowych
    i = os(k, 1);
    j = os(k, 2);
    
    [~,~,Roti] = FromQ(q, i);
    [~,~,Rotj] = FromQ(q, j);
    
    sA = os(k, 3:4)';
    sB = os(k, 5:6)';
    
    [~,dfii,~]=FromQ(dq,i);
    [~,dfij,~]=FromQ(dq,j);
    
    G(ind:ind+1, 1) = Roti * sA * dfii^2 - Rotj * sB * dfij^2; % wzor (2.40)
    
    ind = ind+2;
end

for k=1:size(ps,1) %petla po wszystkich parach postepowych
    G(ind, 1) = 0; % wzor (2.46)
    ind=ind+1;
    
    i = ps(k, 1);
    j = ps(k, 2);
    
    u = ps(k, 4:5)';
    v = [-u(2); u(1)];
    sA = ps(k, 6:7)';
    
    [ri,~,Roti] = FromQ(q, i);
    [rj,~,Rotj] = FromQ(q, j);
    
    [dri,dfii,~]=FromQ(dq,i);
    [drj,dfij,~]=FromQ(dq,j);
    
    G(ind, 1) = (Rotj * v)'*(2 * Om * (drj - dri) * dfij + (rj - ri) * dfij^2 - Roti * sA * (dfij - dfii)^2 ); % ... wzor (2.57)
    
    ind = ind + 1;
end


end

